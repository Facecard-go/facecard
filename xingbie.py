...
def read_excel():
    ...
    cols = sheet1_content1.col_values(1); # 获取第三列内容
    print(cols);
    print(type(cols))
    cols = [i for i in cols if i != '']
    cols = [int(i) for i in cols]
    # ccols = cols[cols.notnull()]
    # print(ccols);
    #cols.loc[cols.Influencing_Event_Event.isnull(), "Influencing_Event_Event"] = "0"
    # 5. 获取单元格内容(三种方式)
    ...
    #         c = c + 1
    #     x = x + 1
    df = {"人群":["蓝人","绿人"],"数量":[men(cols),women(cols)]}
